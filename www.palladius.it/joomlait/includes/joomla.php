	...
	*/
	function initSessionAdmin($option, $task) {
		...
		// check against db record of session
			...
			$this->_db->setQuery( $query );
				...
				if ( $count == 0 ) {
					...
					echo "<script>document.location.href='index.php?mosmsg=Sessione amministratore scaduta'</script>\n";
				exit();
				} else {
			...
			if (ini_get( 'session.auto_start' ) || !ini_get( 'session.use_cookies' )) {
				echo "<script>document.location.href='index.php?mosmsg=Dovete loggarvi. Se le impostazioni session.auto_start del PHP si trova su on oppure le impostazioni session.use_cookies sono su off, dovete correggerle prima di essere in grado di potervi loggare.'</script>\n";
			} else {
		...
		// session id does not correspond to required session format
			echo "<script>document.location.href='index.php?mosmsg=Sessione non valida'</script>\n";
			exit();
		}
	...
	*/
	function _setAdminPaths( $option, $basePath='.' ) {
		...
		if (strpos($option, '/') !== false || strpos($option, '\\') !== false || strpos($option, ':') !== false) {
			mosErrorAlert( 'Accesso non consentito' );
			return;
		}
...
class mosHTML {
  ...
  function writableCell( $folder, $relative=1, $text='', $visible=1 ) {
	$writeable 		= '<b><font color="green">Scrivibile</font></b>';
	$unwriteable 	= '<b><font color="red">Non Scrivibile</font></b>';
...
class mosCategory extends mosDBTable {
	...
	*/
	...
	function check() {
		...
		if (trim( $this->title ) == '') {
			$this->_error = "La categoria deve avere un titolo.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "La categoria deve avere un nome.";
			return false;
		}
		...
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Esistono categorie con questo nome, riprova.";
			return false;
		}
		return true;
	}
...
class mosSection extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "La sezione deve avere un titolo.";
			return false;
		}
		if (trim( $this->name ) == '') {
			$this->_error = "La sezione deve avere un nome.";
			return false;
		}
		// check for existing name
		$query = "SELECT id"
		...
		$xid = intval( $this->_db->loadResult() );
		if ($xid && $xid != intval( $this->id )) {
			$this->_error = "Esiste una sezione con questo nome, riprova.";
			return false;
		}
		return true;
	}
...
class mosModule extends mosDBTable {
	...
	*/
	...
	function check() {
		// check for valid name
		if (trim( $this->title ) == '') {
			$this->_error = "Il modulo deve avere un titolo.";
			return false;
		}
...
class mosSession extends mosDBTable {
	...
	*/
	...
	function insert() {
		...
		if( !$ret ) {
			$this->_error = strtolower(get_class( $this ))."::Storicizzazione fallita <br />" . $this->_db->stderr();
			return false;
		} else {
			return true;
		}
...
function mosWarning($warning, $title='Joomla! Warning') {
	...
	$tip 		.= '<img src="'. $mosConfig_live_site .'/includes/js/ThemeOffice/warning.png" border="0"  alt="attenzione"/></a>';
	...
	*/
	function loadBotGroup( $group ) {
			...
			case 'content':
				...
				;
					...
					if (!($this->_content_mambots = $database->loadObjectList())) {
						//echo "Errore caricamento Mambots: " . $database->getErrorMsg();
						return false;
					}
				}
		...
		if (!($bots = $database->loadObjectList())) {
			//echo "Errore caricamento Mambot: " . $database->getErrorMsg();
			return false;
		}
				break;
		}
...
class mosAdminMenus {
	...
	*/
	function Target( &$row ) {
		$click[] = mosHTML::makeOption( '0', 'Stessa finestra con navigatore browser' );
		$click[] = mosHTML::makeOption( '1', 'Nuova finestra con navigatore browser' );
		$click[] = mosHTML::makeOption( '2', 'Nuova finestra senza navigatore browser' );
	...
	*/
	function MenuLinks( &$lookup, $all=NULL, $none=NULL, $unassigned=1 ) {
		...
		if ( $all ) {
			...
			$mitems[] = mosHTML::makeOption( 0, 'Tutti' );
			// adds space, in select box which is not saved
			$mitems[] = mosHTML::makeOption( -999, '----' );
		}
		if ( $none ) {
			// prepare an array with 'all' as the first item
			$mitems[] = mosHTML::makeOption( -999, 'Nessuno' );
			// adds space, in select box which is not saved
			$mitems[] = mosHTML::makeOption( -999, '----' );
		}
		if ( $unassigned ) {
			// prepare an array with 'all' as the first item
			$mitems[] = mosHTML::makeOption( 99999999, 'Non assegnato' );
			// adds space, in select box which is not saved
			$mitems[] = mosHTML::makeOption( -999, '----' );
		}
	...
	*/
	function Section( &$menu, $id, $all=0 ) {
		...
		$database->setQuery( $query );
		if ( $all ) {
			$rows[] = mosHTML::makeOption( 0, '- Tutte le sezioni -' );
	...
	*/
	function Images( $name, &$active, $javascript=NULL, $directory=NULL ) {
		...
		$images = array(  mosHTML::makeOption( '', '- Seleziona immagine -' ) );
	...
	*/
	function UserSelect( $name, $active, $nouser=0, $javascript=NULL, $order='name', $reg=1 ) {
		...
		$database->setQuery( $query );
		if ( $nouser ) {
			$users[] = mosHTML::makeOption( '0', '- Nessun utente -' );
	...
	*/
	function ComponentCategory( $name, $section, $active=NULL, $javascript=NULL, $order='ordering', $size=1, $sel_cat=1 ) {
		...
		if ( count( $categories ) < 1 ) {
			mosRedirect( 'index2.php?option=com_categories&section='. $section, 'Dobbiamo prima creare almeno una categoria.' );
		}
	...
	*/
	...
	function GetImageFolders( &$temps, $path ) {
		if ( $temps[0]->value != 'None' ) {
			...
			$folders[] 	= mosHTML::makeOption( 'Nesssuna Selezione' );
		}
...
class mosCommonHTML {
	...
	function ContentLegend( ) {
		...
		<tr align="center">
			<td>
			<img src="images/publish_y.png" width="12" height="12" border="0" alt="In attesa" />
			</td>
			<td>
			Pubblicato, ma <u>In attesa</u> |
			</td>
			<td>
			<img src="images/publish_g.png" width="12" height="12" border="0" alt="Visibile" />
			</td>
			<td>
			Pubblicato e <u>Visibile</u> |
			</td>
			<td>
			<img src="images/publish_r.png" width="12" height="12" border="0" alt="Scaduto" />
			</td>
			<td>
			Pubblicato, ma <u>Scaduto</u> |
			</td>
			<td>
			<img src="images/publish_x.png" width="12" height="12" border="0" alt="Scaduto" />
			</td>
			<td>
			Non pubblicato
			</td>
		...
		<tr>
			...
			Clicca sulla icona corrispondente per modificare lo stato.
			</td>
		</tr>
		</table>
		<?php
	...
	function menuLinksContent( &$menus ) {
		...
		foreach( $menus as $menu ) {
			...
			<tr>
				...
				<td>
				<a href="javascript:go2( 'go2menu', '<?php echo $menu->menutype; ?>' );" title="Vai al menu">
			...
			<tr>
				...
				<td>
				<strong>
				<a href="javascript:go2( 'go2menuitem', '<?php echo $menu->menutype; ?>', '<?php echo $menu->id; ?>' );" title="Vai alla voce di menu">
				<?php echo $menu->name; ?>
				</a>
				</strong>
				</td>
			...
			<tr>
				...
				switch ( $menu->published ) {
					case -2:
						echo '<font color="red">Cestinato</font>';
						break;
					case 0:
						echo 'Sospeso';
						break;
					case 1:
					default:
						echo '<font color="green">Pubblicato</font>';
						break;
				}
				?>
				</td>
	...
	*/
	...
	function PublishedProcessing( &$row, $i ) {
		...
		$alt 	= $row->published ? 'Pubblicato' : 'Non pubblicato';
		$action	= $row->published ? 'Sospendi articolo' : 'Pubblica articolo';
