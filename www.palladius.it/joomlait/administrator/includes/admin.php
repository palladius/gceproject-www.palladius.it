...
function josSecurityCheck($width='95%') {
	...
	if ( ini_get('magic_quotes_gpc') != '1' ) {
		$wrongSettingsTexts[] = 'Impostazione PHP magic_quotes_gpc setting è `OFF` invece di `ON`';
	}
	if ( ini_get('register_globals') == '1' ) {
		$wrongSettingsTexts[] = "Impostazione PHP register_globals è `ON` invece di `OFF`";
	}
	if ( RG_EMULATION != 0 ) {
		$wrongSettingsTexts[] = 'Impostazione Joomla! RG_EMULATION è `ON` invece di `OFF` nel file global.php <br /><span style="font-weight: normal; font-style: italic; color: #666;">`ON` predefinito per ragioni di compatibilità</span>';
	}
	...
	if ( count($wrongSettingsTexts) ) {
		...
		<div style="clear: both; margin: 3px; margin-top: 10px; padding: 5px 15px; display: block; float: left; border: 1px solid #cc0000; background: #ffffcc; text-align: left; width: <?php echo $width;?>;">
			<p style="color: #CC0000;">
				Le seguenti impostazioni PHP Server Security Settings non sono ottimali per la <strong>Sicurezza</strong> raccomandiamo quindi di modificarle:
			</p>
			...
			<p style="color: #666;">
				Prendere visione del post nel forum <a href="http://www.joomla.org/security10" target="_blank" style="color: blue; text-decoration: underline">Official Joomla! Server Security </a> per maggiori informazioni.
			</p>
		</div>
		<?php
	}
