...
function editSection( $uid=0, $scope='', $option ) {
	...
	if ($row->isCheckedOut( $my->id )) {
		$msg = 'La sezione '. $row->title .' è al momento in fase di modifica da parte di un altro amministratore';
			...
			for( $i = 0; $i < $count; $i++ ) {
				switch ( $menus[$i]->type ) {
					case 'content_section':
						$menus[$i]->type = 'Tabella Sezione';
						break;

					case 'content_blog_section':
						$menus[$i]->type = 'Blog Sezione';
						break;

					case 'content_archive_section':
						$menus[$i]->type = 'Archivio Blog Sezione';
						break;
				}
			}
		} else {
	...
	$types[] = mosHTML::makeOption( 'content_section', 'Lista Sezione' );
	$types[] = mosHTML::makeOption( 'content_blog_section', 'Blog Sezione' );
	$types[] = mosHTML::makeOption( 'content_archive_section', 'Archivio Blog Sezione' );
...
function removeSections( $cid, $scope, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Seleziona una sezione da cancellare'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( ', ', $err );
		$msg = 'Sezione(i): '. $cids .' impossibile da rimuovere in quanto contiene categorie';
		mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
	}
	...
	$names = implode( ', ', $name );
	$msg = 'Sezione(i): '. $names .' cancellata correttamente';
	mosRedirect( 'index2.php?option='. $option .'&scope='. $scope, $msg );
}
...
function publishSections( $scope, $cid=null, $publish=1, $option ) {
	...
	if ( !is_array( $cid ) || count( $cid ) < 1 ) {
		...
		echo "<script> alert('Seleziona una sezione da $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function copySectionSave( $sectionid ) {
	...
	$msg = 'Sezione contenuti selezionata copiata nella sezione '. $title .'';
...
function menuLink( $id ) {
	...
	$msg = $name .' ( '. $menutype .' ) nel menu: '. $menu .' creato correttamente';
