...
class HTML_banners {
	...
	function showBanners( &$rows, &$pageNav, $option ) {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			$alt 	= $row->showBanner ? 'Pubblicato' : 'Sospeso';
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					...
					?>
					<a href="<?php echo $link; ?>" title="Modifica banner">
					<?php echo $row->name; ?>
					</a>
					<?php
				}
	...
	function bannerForm( &$_row, &$lists, $_option ) {
		...
		<tr>
			<th>
			<?php echo $_row->cid ? 'Modifica' : 'Nuovo';?>
			Banner:
			</th>
		</tr>
		</table>
		...
		<tr>
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			Illimitati <input type="checkbox" name="unlimited" <?php echo $unlimited;?> />
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr >
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input name="reset_hits" type="button" class="button" value="Resetta Click" onclick="submitbutton('resethits');" />
			</td>
		</tr>
		<tr>
			<td valign="top">
			Codice banner personale:
			</td>
			<td>
...
class HTML_bannerClient {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					...
					?>
					<a href="<?php echo $link; ?>" title="Modifica cliente">
					<?php echo $row->name; ?>
					</a>
					<?php
				}
	...
	function bannerClientForm( &$row, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.name.value == "") {
				alert( "Compilare il campo nome cliente." );
			} else if (form.contact.value == "") {
				alert( "Compilare il campo nome contatto." );
			} else if (form.email.value == "") {
				alert( "Compilare il campo E-mail contatto." );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			<th>
			<small>
			<?php echo $row->cid ? 'Modifica' : 'Nuovo';?>
			</small>
			Cliente:
			</th>
		</tr>
