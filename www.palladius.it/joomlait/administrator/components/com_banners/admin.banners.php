...
function editBanner( $bannerid, $option ) {
  ...
  if ( $bannerid ){
	...
	$clientlist[] 	= mosHTML::makeOption( '0', 'Seleziona Cliente', 'cid', 'name' );
...
function saveBanner( $task ) {
	...
	$msg = 'Informazioni banner salvate';
	if (!$row->bind( $_POST )) {
...
function publishBanner( $cid, $publish=1 ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		...
		echo "<script> alert('Seleziona un elemento da $action'); window.history.go(-1);</script>\n";
		exit();
	}
...
function editBannerClient( $clientid, $option ) {
	...
	if ($row->checked_out && $row->checked_out != $my->id) {
		$msg = 'Il cliente [ '. $row->name. ' ]  al momento in fase di modifica da parte di un altro amministratore.';
...
function removeBannerClients( $cid, $option ) {
	...
	for ($i = 0; $i < count($cid); $i++) {
		...
		if ($count != 0) {
			mosRedirect( "index2.php?option=$option&task=listclients",
			"Impossibile cancellare il cliente in quanto ha ancora banner attivi" );
		} else {
