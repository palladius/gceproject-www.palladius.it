...
function saveMambot( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Modifiche al Mambot salvate correttamente: '. $row->name;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=editA&hidemainmenu=1&id='. $row->id, $msg );

		case 'save':
		default:
			$msg = 'Mambot salvato correttamente: '. $row->name;
...
function editMambot( $option, $uid, $client ) {
	...
	if ($uid) {
		...
		$lists['ordering'] = '<input type="hidden" name="ordering" value="'. $row->ordering .'" />I nuovi oggetti assumo di default l&acute;ultima posizione. Questo ordine pu&ograve; essere modificato dopo il salvataggio.';
	}
...
function removeMambot( &$cid, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Seleziona un mambot da cancellare'); window.history.go(-1);</script>\n";
		exit;
	}
...
function publishMambot( $cid=null, $publish=1, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		...
		echo "<script> alert('Seleziona un mambot da $action'); window.history.go(-1);</script>\n";
		exit;
	}
