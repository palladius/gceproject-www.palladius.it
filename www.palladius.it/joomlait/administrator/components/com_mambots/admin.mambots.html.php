...
class HTML_modules {
	...
	*/
	function showMambots( &$rows, $client, &$pageNav, $option, &$lists, $search ) {
		...
		<tr>
			<th class="modules">
			Gestione mambot <small><small>[ <?php echo $client == 'admin' ? 'Amministratore' : 'Sito';?> ]</small></small>
			</th>
			<td>
			Filtro:
			</td>
	...
	*/
	function editMambot( &$row, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if (form.name.value == "") {
				alert( "Il mambot deve avere un nome" );
			} else if (form.element.value == "") {
				alert( "il mambot deve avere un file" );
			} else {
				submitform(pressbutton);
			}
		}
		...
		<tr>
			<th class="mambots">
			<small>
			<?php echo $row->id ? 'Modifica' : 'Nuovo';?>: Mambot sito
			</small>
			<?php echo $row->nameA; ?>
			</th>
		</tr>
