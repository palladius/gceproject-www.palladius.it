...
function viewTemplates( $option, $client ) {
	...
	foreach($templateDirs as $templateDir) {
		...
		foreach($xmlFilesInDir as $xmlfile) {
			...
			$row->creationdate = $element ? $element->getText() : 'Sconosciuto';

			$element 		= &$root->getElementsByPath('author', 1);
			$row->author 	= $element ? $element->getText() : 'Sconosciuto';
...
function removeTemplate( $cid, $option, $client ) {
	...
	if ($cur_template == $cid) {
		mosErrorAlert( "Impossibile cancellare il template in uso" );
	}
...
function editTemplateSource( $p_tname, $option, $client ) {
	...
	if ( $fp = fopen( $file, 'r' ) ) {
		...
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: impossibile aprire '. $file );
	}
}
...
function saveTemplateSource( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: Nessun template specificato.' );
	}
	if ( !$filecontent ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: Contenuto vuoto.' );
	}
	...
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option , 'Operazione fallita: '. $file .' non scrivibile.' );
	}
...
function saveTemplateCSS( $option, $client ) {
	...
	if ( !$template ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: Nennun template specificato.' );
	}
	...
	clearstatcache();
	if ( is_writable( $file ) == false ) {
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: file non scrivibile.' );
	}
	...
	if ($fp = fopen ($file, 'w')) {
		...
		if ($enable_write) @chmod($file, $oldperms);
		mosRedirect( 'index2.php?option='. $option .'&client='. $client, 'Operazione fallita: Impossibile aprire file per la scrittura.' );
	}
...
function savePositions( $option ) {
	...
	mosRedirect( 'index2.php?option='. $option .'&task=positions', 'Posizione salvata' );
}
?>