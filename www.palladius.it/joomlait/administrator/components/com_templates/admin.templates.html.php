...
class HTML_templates {
	...
	function showTemplates( &$rows, &$pageNav, $option, $client ) {
		...
		function showInfo(name, dir) {
			...
			if (document.adminForm.doPreview.checked) {
				...
				var html=name;
				html = '<br /><img border="1" src="'+src+'" name="imagelib" alt="Nessuna anteprima disponibile" width="206" height="145" />';
		...
		<tr>
			<th class="templates">
			Gestione template <small><small>[ <?php echo $client == 'admin' ? 'Amministratore' : 'Sito';?> ]</small></small>
			</th>
		...
		for ( $i=0, $n = count( $rows ); $i < $n; $i++ ) {
			...
			<tr class="<?php echo 'row'. $k; ?>">
				...
				if ( $client == 'admin' ) {
					...
					if ( $row->published == 1 ) {
						?>
					<img src="images/tick.png" alt="Pubblicato">
						<?php
					} else {
						?>
						&nbsp;
					...
					if ( $row->published == 1 ) {
						?>
						<img src="images/tick.png" alt="Predefinito">
						<?php
					} else {
						?>
						&nbsp;
					...
					if ( $row->assigned == 1 ) {
						?>
						<img src="images/tick.png" alt="Assegnato" />
						<?php
					} else {
						?>
						&nbsp;
	...
	*/
	function editTemplateSource( $template, &$content, $option, $client ) {
		...
		<tr>
			<td width="290"><table class="adminheading"><tr><th class="templates">Editor HTML template</th></tr></table></td>
			<td width="220">
				<span class="componentheading">il file index.php &#232; :
				<b><?php echo is_writable($template_path) ? '<font color="green"> Scrivibile</font>' : '<font color="red"> Non scrivibile </font>' ?></b>
				</span>
			</td>
<?php
...
?>
			<td>
				...
				<label for="enable_write">Oltrepassa protezione di scrittura quando salviamo</label>
			</td>
<?php
				} // if
			} // if
?>
	...
	*/
	function editCSSSource( $template, &$content, $option, $client ) {
		...
		<table cellpadding="1" cellspacing="1" border="0" width="100%">
		<tr>
			<td width="280"><table class="adminheading"><tr><th class="templates">Editor CSS template</th></tr></table></td>
			<td width="260">
				<span class="componentheading">il file template_css.css &#232; :
				<b><?php echo is_writable($css_path) ? '<font color="green"> Scrivibile</font>' : '<font color="red"> Unwriteable</font>' ?></b>
				</span>
			</td>
<?php
	...
	*/
	function assignTemplate( $template, &$menulist, $option ) {
		...
		<tr>
			...
			Assiegna template <?php echo $template; ?> alla voce di menu
			</th>
		</tr>
		<tr>
