...
class HTML_poll {
	...
	function showPolls( &$rows, &$pageNav, $option ) {
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td>
				<a href="<?php echo $link; ?>" title="Modifica sondaggio">
				<?php echo $row->title; ?>
				</a>
				</td>
				<td align="center">
	...
	function editPoll( &$row, &$options, &$lists ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.title.value == "") {
				alert( "Il sondaggio deve avere un titolo" );
			} else if( isNaN( parseInt( form.lag.value ) ) ) {
				alert( "Il sondaggio non deve avere un lag time a zero" );
			//} else if (form.menu.options.value == ""){
			//	alert( "Il sondaggio deve avere delle pagine." );
			//} else if (form.adminForm.textfieldcheck.value == 0){
			//	alert( "Il sondaggio deve avere delle opzioni." );
			} else {
				submitform( pressbutton );
			}
		}
		...
		<tr>
			<th>
			<small>
			<?php echo $row->id ? 'Modifica' : 'Nuovo';?>: Sondaggio
			</small>
			</th>
		</tr>
		</table>
