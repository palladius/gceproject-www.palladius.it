...
function viewTrash( $option ) {
	...
	;
		...
		for ( $i = 0; $i < $num-1; $i++ ) {
			if ( ( $content[$i]->sectionid == 0 ) && ( $content[$i]->catid == 0 ) ) {
				$content[$i]->sectname = 'Contenuto Statico';
			}
		}
	} else {
	...
	$listselect = array();
	$listselect[] = mosHTML::makeOption( 'content', 'Contenuti' );
	$listselect[] = mosHTML::makeOption( 'menu', 'Voci di menu' );
	$selected = "all";
...
function deleteTrash( $cid, $option ) {
	...
	$msg = $total. " Oggetto(i) cancellati correttamente";
	mosRedirect( "index2.php?option=$option&mosmsg=". $msg ."" );
}


/**
* Compiles a list of the items you have selected to permanently delete
*/
...
function restoreTrash( $cid, $option ) {
	...
	$msg = $total. " Oggetto(i) ripristinati correttamente";
	mosRedirect( "index2.php?option=$option&mosmsg=". $msg ."" );
}
?>