...
function publishLanguage( $p_lname, $option ) {
	...
	if ($fp = fopen('../configuration.php','w')){
		...
		fclose($fp);
		mosRedirect('index2.php?option=com_languages',"Lingua modificata correttamente! $p_lname");
	} else {
		mosRedirect('index2.php?option=com_languages','Errore!');
	}
...
function removeLanguage( $cid, $option, $client = 'admin' ) {
	...
	if ($cur_language == $cid) {
		mosErrorAlert( "Impossibile cancellare la lingua in uso." );
	}
...
function editLanguageSource( $p_lname, $option) {
	...
	if ($fp = fopen( $file, "r" )) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Operazione fallita: Impossibile aprire  $file" );
	}
}
...
function saveLanguageSource( $option ) {
	...
	if (!$language) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operazione fallita: Nessuna lingua specificata." );
	}
	if (!$filecontent) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operazione fallita: Contenuto vuoto." );
	}
	...
	if (is_writable( $file ) == false) {
		mosRedirect( "index2.php?option=$option&mosmsg=Operazione fallita: File non scrivibile." );
	}
	...
	if ($fp = fopen ($file, "w")) {
		...
		mosRedirect( "index2.php?option=$option&mosmsg=Operazione fallita: Apertura file per la scrittura non possibile." );
	}
}
?>