	...
	function editCategory( &$menu, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if ( !$menu->id ) {
				...
				?>
				if ( form.name.value == '' ) {
					alert( 'la voce di menu deve avere un titolo' );
				} else {
					submitform( pressbutton );
				}
				<?php
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Modifica' : 'Aggiungi';?> Voce di menu :: Tabella - Categoria Contatti
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					...
					<?php
					if ( !$menu->id ) {
						echo mosToolTip( 'Se lasciato in bianco prende automaticamente il nome della categoria' );
					}
					?>
					</td>
				</tr>
