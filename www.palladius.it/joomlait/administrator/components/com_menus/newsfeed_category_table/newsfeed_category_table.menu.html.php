	...
	function editCategory( &$menu, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			if ( !$menu->id ) {
				...
				?>
				if ( form.name.value == '' ) {
					alert( 'Questa voce di menu deve avere un tittolo' );
				} else {
					submitform( pressbutton );
				}
				<?php
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Modifica' : 'Aggiungi';?> Voce di menu :: Tabella - Categoria News feed
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					...
					<?php
					if ( !$menu->id ) {
						echo mosToolTip( 'se lasciato in bianco prende come nome il nome della categoria in automatico' );
					}
					?>
					</td>
				</tr>
