...
class content_archive_section_menu_html {
	...
	function editSection( &$menu, &$lists, &$params, $option ) {
			...
			if ( !$menu->id ) {
				...
				if ( form.name.value == '' ) {
					if ( form.componentid.value == 0 ) {
						form.name.value = "tuttele sezioni";
					} else {
		...
		<tr>
			<th>
			<?php echo $menu->id ? 'Modifica' : 'Aggiungi';?> Voce di menu :: Blog - Archivio Contenuto Sezione
			</th>
		</tr>
		</table>
		...
		<tr valign="top">
			<td width="60%">
				...
				<tr>
					...
					<?php
					if ( !$menu->id ) {
						echo mosToolTip( 'Se lasciato in bianco prende automaticamente il nome della sezione' );
					}
					?>
					</td>
				</tr>
				<tr>
					<td valign="top" align="right">Sezione:</td>
					<td colspan="2">
