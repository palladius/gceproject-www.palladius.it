...
if (is_int(strpos ($listdir, "..")) && $listdir != '') {
	mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "NESSUN HACKING GRAZIE" );
}
...
switch ($task) {
	...
	case 'newdir':
		if (ini_get('safe_mode')=='On') {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Creazione directory non permessa in regime di SAFE MODE e questo causa problemi." );
		} else {
...
function create_folder($dirPath) {
	...
	if(strlen($folder_name) >0) {
		if (eregi("[^0-9a-zA-Z_]", $folder_name)) {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Il nome directory deve contenere solo caratteri alfanumerici senza spazi." );
		}
...
function delete_folder($listdir) {
	...
	if ($entry_count < 1) {
		...
		echo '<font color="red">Impossibile cancellare: non vuota!</font>';
	}
}
...
function do_upload($file, $dest_dir) {
	...
	if (empty($file['name'])) {
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Non selezionato alcun file da caricare" );
	}
		if (file_exists($dest_dir.$file['name'])) {
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Caricamento FALLITO. Nessun file selezionato oppure un file con questo nome esiste in archivio " );
		}
        ...
        if(!$noMatch){
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], 'Tipo di file non supportato' );
        }

		if (!move_uploaded_file($file['tmp_name'], $dest_dir.strtolower($file['name']))){
		mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Caricamento FALLITO" );
	} else {
			mosChmod($dest_dir.strtolower($file['name']));
			mosRedirect( "index2.php?option=com_media&listdir=".$_POST['dirPath'], "Caricamento completato" );
		}
