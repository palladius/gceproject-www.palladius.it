...
function showUsers( $option ) {
	...
	. "\n AND name != 'USERS'"
	;
	$types[] = mosHTML::makeOption( '0', '- Seleziona gruppo -' );
	$database->setQuery( $query );
	$types = array_merge( $types, $database->loadObjectList() );
	$lists['type'] = mosHTML::selectList( $types, 'filter_type', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"', 'value', 'text', "$filter_type" );

	// get list of Log Status for dropdown filter
	$logged[] = mosHTML::makeOption( 0, '- Seleziona stato - ');
	$logged[] = mosHTML::makeOption( 1, 'Connesso');
...
function saveUser( $task ) {
	...
	if ($isNew) {
		...
		if ( $row->gid != $original->gid ) {		
			...
			if ( $count <= 1 ) {
				...
				echo "<script> alert('Impossibile modificare questo gruppo utenti in quanto attivo solo un Super Amministratore per questo sito'); window.history.go(-1); </script>\n";
				exit();
			}
		}
			...
			if (( $user_group == 'super administrator' && $my->gid != 25) ) {
				...
				echo "<script> alert('Impossibile modificare questo gruppo utenti se non siete un Super Amministratore di questo sito'); window.history.go(-1); </script>\n";
				exit();
			} else if ( $my->gid == 24 && $original->gid == 24 ) {
				// disallow change of super-Admin by non-super admin
				echo "<script> alert('Impossibile modificare il gruppo di un Amministratore se non siete un Super Amministratore di questo sito'); window.history.go(-1); </script>\n";
				exit();
	...
	if ( $row->gid == 25 && $my->gid != 25 ) {
		...
		echo "<script> alert('Impossibile creare un utente con questo gruppo di accesso,  solo il Super Amministratore ha questo potere'); window.history.go(-1); </script>\n";
		exit();
	}
	*/
	...
	if (!in_array($row->gid,getGIDSChildren($my->gid))) {
		// disallow creation of Super Admin by non Super Admin users
		echo "<script> alert('Impossibile creare un utente in questo gruppo e con questo livello, solo il Super Amministratore ha questo potere'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Modifiche utente salvate correttamte: '. $row->name;
			mosRedirect( 'index2.php?option=com_users&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Utente salvato correttamente: '. $row->name;
...
function removeUsers( $cid, $option ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Seleziona un oggetto da cancellare'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if ( !$msg && count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			// cannot delete Super Admin where it is the only one that exists
				$msg = "Impossibile cancellare questo Super Amministratore in quanto unico Super Amministratore atttivo del vostro sito";
			} else {
				// delete user
				$obj->delete( $id );
				$msg = $obj->getError();
	...
	if ( count( $cid ) ) {
		...
		foreach ($cid as $id) {
			...
			if ( $this_group == 'super administrator' && $my->gid != 25 ) {
				$msg = "Impossibile cancellare il Super Amministratore";
 			} else if ( $id == $my->id ){
 				$msg = "Non puoi cancellare te stesso!";
 			} else if ( ( $this_group == 'administrator' ) && ( $my->gid == 24 ) ){
 				$msg = "Impossibile cancellare un altro `Amministratore` solo il `Super Amministratore` ha questo potere";
			} else {
				$obj->load( $id );
				$count = 2;
				if ( $obj->gid == 25 ) {
				...
				if ( $count <= 1 && $obj->gid == 25 ) {
				// cannot delete Super Admin where it is the only one that exists
					$msg = "Impossibile cancellare il Super Amministratore in quanto unico attivo per questo sito";
				} else {
					// delete user
				$obj->delete( $id );
				$msg = $obj->getError();
...
function changeUserBlock( $cid=null, $block=1, $option ) {
	...
	$action = $block ? 'bloccare' : 'sbloccare';
...
function changeUserBlock( $cid=null, $block=1, $option ) {
	...
	if (count( $cid ) < 1) {
		$action = $block ? 'bloccare' : 'sbloccare';
		echo "<script> alert('Seleziona un oggetto da $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function logoutUser( $cid=null, $option, $task ) {
	...
	if ( is_array( $cid ) ) {
		if (count( $cid ) < 1) {
			mosRedirect( 'index2.php?option='. $option, 'Selezionare un utente' );
		}
		...
		// check to see whether a Administrator is attempting to log out a Super Admin
		if ( $my->gid == 24 && $temp->gid == 25 ) {
			echo "<script> alert('Impossibile sconnettere un Super Amministratore'); window.history.go(-1); </script>\n";
			exit();
		}
		$ids = 'userid=' . (int) $cid;
	}
	...
	if (is_array( $cid ) && count( $cid )) {
		...
		foreach ($cid as $id) {
			...
			if ( !$allowActionToMyself && $id == $my->id ){
 				$msg .= 'Impossibile '. $actionName .' te stesso!';
 			} else if (($obj->gid == $my->gid && !in_array($my->gid, array(24, 25))) || ($obj->gid && !in_array($obj->gid,getGIDSChildren($my->gid)))) {
				$msg .= 'Impossibile '. $actionName .' un `'. $this_group .'`. Solo gli utenti di alto livello hanno questo potere. ';
			}
		}
	}
