...
// PRE-PROCESS SOME LISTS
	...
	$show_hide = array(
		mosHTML::makeOption( 1, 'Nascondi' ),
		mosHTML::makeOption( 0, 'Mostra' ),
	);

	$show_hide_r = array(
		mosHTML::makeOption( 0, 'Nascondi' ),
		mosHTML::makeOption( 1, 'Mostra' ),
	);
...
// SERVER SETTINGS
	...
	$session = array(
		mosHTML::makeOption( 0, 'Livello 3 Sicurezza - Predefinito & Massima' ),
		mosHTML::makeOption( 1, 'Livello 2 Sicurezza - Permessi per IP proxy' ),
		mosHTML::makeOption( 2, 'Livello 1 Sicurezza - Retrocompatibilit&agrave;' )
	);
	...
	$errors = array(
		mosHTML::makeOption( -1, 'Default Sistema' ),
		mosHTML::makeOption( 0, 'Nessuno' ),
		mosHTML::makeOption( E_ERROR|E_WARNING|E_PARSE, 'Semplice' ),
		mosHTML::makeOption( E_ALL , 'Massimo' )
	);
...
// CONTENT SETTINGS
	...
	if (is_writable( "$mosConfig_absolute_path/media/" )) {
		...
		$lists['hidePdf'] 			= '<input type="hidden" name="config_hidepdf" value="1" /><strong>Nascosto</strong>';
	}
...
function saveconfig( $task ) {
	...
	if ( $fp = fopen($fname, 'w') ) {
		...
		$msg = 'I dettagli configurazione sono stati aggiornati';
		...
		mosRedirect( 'index2.php', 'Avvenuto un errore! Impossibile aprire il file configurazione per la scrittura!' );
	}
}
?>