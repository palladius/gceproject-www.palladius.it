...
function showCategories( $section, $option ) {
	...
	if (intval( $section ) > 0) {
		...
		$section_name = 'Componente: '. $section_name;
	} else {
		$table 	= $section;
		$where 	= "\n WHERE c.section = " . $database->Quote( $section );
		$type 	= 'other';
...
function editCategory( $uid=0, $section='' ) {
	...
	if (!$sections && $type != 'other') {
		echo "<script> alert('Dovete almeno avere una Sezione prima di creare una Categoria'); window.history.go(-1); </script>\n";
		exit();
	}
	...
	if ($row->checked_out && $row->checked_out != $my->id) {
		mosRedirect( 'index2.php?option=categories&section='. $row->section, 'La categoria '. $row->title .' al momento in fase di modifica da parte di altro amministratore' );
	}
	...
	if ( $uid ) {
		...
		switch ( $row->section ) {
			case 'com_weblinks':
				...
				$link 	= 'Tabella - Categoria Weblink';
				break;

			case 'com_newsfeeds':
				$and 	= "\n AND type = 'newsfeed_category_table'";
				$link 	= 'Tabella - Categoria Newsfeeds';
				break;

			case 'com_contact_details':
				$and 	= "\n AND type = 'contact_category_table'";
				$link 	= 'Tabella - Categoria Contatti';
				break;
			...
			for( $i = 0; $i < $count; $i++ ) {
				switch ( $menus[$i]->type ) {
					case 'content_category':
						$menus[$i]->type = 'Tabella - Categoria';
						break;

					case 'content_blog_category':
						$menus[$i]->type = 'Blog - Categoria';
						break;

					case 'content_archive_category':
						$menus[$i]->type = 'Archivio - Blog Categoria';
						break;
				}
			}
			$lists['links']	= 1;
	...
	$types[] = mosHTML::makeOption( '', 'Seleziona tipo' );
	if ($row->section == 'com_contact_details') {
		$types[] = mosHTML::makeOption( 'contact_category_table', 'Tabella Categoria Contatto' );
	} else
	if ($row->section == 'com_newsfeeds') {
		$types[] = mosHTML::makeOption( 'newsfeed_category_table', 'Tabella Categoria Newsfeed' );
	} else
	if ($row->section == 'com_weblinks') {
		$types[] = mosHTML::makeOption( 'weblink_category_table', 'Tabella Categoria Weblink' );
	} else {
		$types[] = mosHTML::makeOption( 'content_category', 'Tabella Categoria Contenuti' );
		$types[] = mosHTML::makeOption( 'content_blog_category', 'Categoria Blog Contenuti' );
		$types[] = mosHTML::makeOption( 'content_archive_category', 'Archivio Categoria Blog Contenuti ' );
	} // if
	...
	if ( $row->section > 0 || $row->section == 'content' ) {
		...
		$folders[] 	= mosHTML::makeOption( '*2*', 'Usa configurazione sezione'  );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '*1*', 'Tutte'  );
		$folders[] 	= mosHTML::makeOption( '*0*', 'Nessuna' );
		$folders[] 	= mosHTML::makeOption( '*#*', '---------------------' );
		$folders[] 	= mosHTML::makeOption( '/' );
...
function removeCategories( $section, $cid ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Seleziona una categoria da cancellare'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	if (count( $err )) {
		$cids = implode( "\', \'", $err );
		$msg = 'Le Categorie: '. $cids .' non possono essere rimosse se contengono oggetti';
...
function publishCategories( $section, $categoryid=null, $cid=null, $publish=1 ) {
	...
	if (count( $cid ) < 1) {
		$action = $publish ? 'pubblicare' : 'sospendere';
		echo "<script> alert('Seleziona una categoria da $action'); window.history.go(-1);</script>\n";
		exit;
	}
...
function moveCategorySelect( $option, $cid, $sectionOld ) {
	...
	if (!is_array( $cid ) || count( $cid ) < 1) {
		echo "<script> alert('Seleziona un oggetto da spostare'); window.history.go(-1);</script>\n";
		exit;
	}
...
function moveCategorySave( $cid, $sectionOld ) {
	...
	if ( !$sectionMove ) {
		mosRedirect( 'index.php?option=com_categories&mosmsg=Avvenuto un errore' );
	}
	...
	$msg = ( (count($cid) - 1) ? 'Categorie' : 'Categoria' ) .' spostate in '. $sectionNew->name;		
...
function copyCategorySave( $cid, $sectionOld ) {
	...
	$msg = ( (count($cid) - 1) ? 'Categorie' : 'Categoria' ) .' copiate in '. $sectionNew->name;
	mosRedirect( 'index2.php?option=com_categories&section='. $sectionOld .'&mosmsg='. $msg );
}
