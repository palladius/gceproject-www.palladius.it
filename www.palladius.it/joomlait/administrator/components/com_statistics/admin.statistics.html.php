...
class HTML_statistics {
	function show( &$browsers, &$platforms, $tldomains, $bstats, $pstats, $dstats, $sorts, $option ) {
		...
		<tr>
			<th class="browser">Statistiche Browser, OS, Domini</th>
		</tr>
		</table>
		...
		<?php
		$tabs->endTab();
		$tabs->startTab("Stat. OS","os-page");
		?>
		<table class="adminlist">
		<tr>
			<th align="left">&nbsp;Sistemi operativi <?php echo $sorts['o_agent'];?></th>
			<th>&nbsp;</th>
		...
		<?php
		$tabs->endTab();
		$tabs->startTab("Stat. Domini","domain-page");
		?>
		<table class="adminlist">
		<tr>
			<th align="left">&nbsp;Domini <?php echo $sorts['d_agent'];?></th>
			<th>&nbsp;</th>
	...
	function pageImpressions( &$rows, $pageNav, $option, $task ) {
		...
		<tr>
			<th width="100%" class="impressions">Statistiche Page Impressions</th>
		</tr>
		</table>
	...
	function showSearches( &$rows, $pageNav, $option, $task, $showResults ) {
		...
		<table cellpadding="4" cellspacing="0" border="0" width="100%" class="adminheading">
			<tr>
				...
				<span class="componentheading">raporto :
				<?php echo $mainframe->getCfg( 'enable_log_searches' ) ? '<b><font color="green">Abilitato</font></b>' : '<b><font color="red">Disabilitato</font></b>' ?>
				</span>
				</th>
			<td align="right">
				<?php
				if ( !$showResults ) {
					echo mosWarning('Attivando questa opzione sono possibili dei rallentamenti dovuti al grande numero di query richieste');
				}
				?>
			</td>
			<td align="right">
				...
				if ( $showResults ) {
					?>
					<input name="search_results" type="button" class="button" value="Nascondi risultati ricerca" onclick="submitbutton('searches');">
					<?php
				} else {
					?>
					<input name="search_results" type="button" class="button" value="Mostra risultati ricerca" onclick="submitbutton('searchesresults');">
					<?php
				}
				?>
			</td>
