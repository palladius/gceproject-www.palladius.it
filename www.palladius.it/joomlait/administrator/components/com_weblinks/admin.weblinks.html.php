		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				if ( $row->checked_out && ( $row->checked_out != $my->id ) ) {
					...
					?>
					<a href="<?php echo $link; ?>" title="Modifica Weblink">
					<?php echo $row->title; ?>
					</a>
					<?php
				}
	...
	*/
	function editWeblink( &$row, &$lists, &$params, $option ) {
		...
		function submitbutton(pressbutton) {
			...
			// do field validation
			if (form.title.value == ""){
				alert( "Il collegamento web deve avere un titolo" );
			} else if (form.catid.value == "0"){
				alert( "Dovete selezionare una categoria." );
			} else if (form.url.value == ""){
				alert( "Dovete indicare una url." );
			} else {
				submitform( pressbutton );
			}
		}
