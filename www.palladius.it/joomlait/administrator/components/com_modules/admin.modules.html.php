	...
	*/
	function showModules( &$rows, $myid, $client, &$pageNav, $option, &$lists, $search ) {
		...
		<tr>
			...
			Gestione moduli <small><small>[ <?php echo $client == 'admin' ? 'Amministratore' : 'Sito';?> ]</small></small>
			</th>
			<td width="right">
		...
		for ($i=0, $n=count( $rows ); $i < $n; $i++) {
			...
			<tr class="<?php echo "row$k"; ?>">
				...
				<td align="left">
				<?php echo $row->module ? $row->module : "Utente";?>
				</td>
			</tr>
			<?php
			$k = 1 - $k;
	...
	*/
	function editModule( &$row, &$orders2, &$lists, &$params, $option ) {
		...
		<tr>
			<th class="modules">
			<small>
			<?php echo $row->id ? 'Modifica' : 'Nuovo';?>:
			Modulo: <?php echo $lists['client_id'] ? 'Amministratore' : 'Sito';?>
			</small>
			<?php echo $row->titleA; ?>
			</th>
		</tr>
