...
function saveModule( $option, $client, $task ) {
	...
	switch ( $task ) {
		case 'apply':
			$msg = 'Modifiche al modulo salvate correttamente: '. $row->title;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client .'&task=editA&hidemainmenu=1&id='. $row->id, $msg );
			break;

		case 'save':
		default:
			$msg = 'Modulo salvato correttamente: '. $row->title;
			mosRedirect( 'index2.php?option='. $option .'&client='. $client, $msg );
			break;
	}
}
...
function editModule( $option, $uid, $client ) {
	...
	if ( $uid ) {
		...
		$lookup = array( mosHTML::makeOption( 0, 'Tutti' ) );
	}
...
function removeModule( &$cid, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Seleziona un modulo da cancellare'); window.history.go(-1);</script>\n";
		exit;
	}
	...
	foreach ($rows as $row) {
		...
		if ( $row->module == 'mod_mainmenu' ) {
			if ( strstr( $row->params, 'mainmenu' ) ) {
				echo "<script> alert('Impossibile cancellare il modulo mod_mainmenu mostrato come \'mainmenu\' in quanto modulo del core.'); window.history.go(-1); </script>\n";
				exit;
			}
		}
	}
	...
	if (count( $err )) {
		...
		echo "<script>alert('Modulo(i): \'$cids\' impossibile da rimuovere può essere solo sospeso in quanto modulo Joomla!.');</script>\n";
	}
...
function publishModule( $cid=null, $publish=1, $option, $client ) {
	...
	if (count( $cid ) < 1) {
		...
		echo "<script> alert('Seleziona un modulo da $action'); window.history.go(-1);</script>\n";
		exit;
	}
