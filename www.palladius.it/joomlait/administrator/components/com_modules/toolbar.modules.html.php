...
class TOOLBAR_modules {
	...
	*/
	function _EDIT( $cur_template, $publish ) {
		...
		?>
			<td>
				<a class="toolbar" href="#" onClick="if (typeof document.adminForm.content == 'undefined') { alert('Anteprima possibile solo per `nuovi` moduli.'); } else { var content = document.adminForm.content.value; content = content.replace('#', '');  var title = document.adminForm.title.value; title = title.replace('#', ''); window.open('popups/modulewindow.php?title=' + title + '&content=' + content + '&t=<?php echo $cur_template; ?>', 'win1', 'status=no,toolbar=no,scrollbars=auto,titlebar=no,menubar=no,resizable=yes,width=200,height=400,directories=no,location=no'); }" >
					<img src="images/preview_f2.png" alt="Anteprima" border="0" name="preview" align="middle">
					Anteprima</a>
			</td>
		<?php
		mosMenuBar::spacer();
		...
		if ( $id ) {
			...
			mosMenuBar::cancel( 'cancel', 'Chiudi' );
		} else {
			mosMenuBar::cancel();
		}
		mosMenuBar::spacer();
	...
	function _DEFAULT() {
		...
		mosMenuBar::spacer();
		mosMenuBar::custom( 'copy', 'copy.png', 'copy_f2.png', 'Copia', true );
		mosMenuBar::spacer();
