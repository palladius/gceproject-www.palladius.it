...
function view( $option ) {
	...
	$ordering[] = mosHTML::makeOption( 'c.ordering ASC', 'Ordine asc' );
	$ordering[] = mosHTML::makeOption( 'c.ordering DESC', 'Ordine disc' );
	$ordering[] = mosHTML::makeOption( 'c.id ASC', 'ID asc' );
	$ordering[] = mosHTML::makeOption( 'c.id DESC', 'ID disc' );
	$ordering[] = mosHTML::makeOption( 'c.title ASC', 'Titolo asc' );
	$ordering[] = mosHTML::makeOption( 'c.title DESC', 'Titolo disc' );
	$ordering[] = mosHTML::makeOption( 'c.created ASC', 'Data asc' );
	$ordering[] = mosHTML::makeOption( 'c.created DESC', 'Data disc' );
	$ordering[] = mosHTML::makeOption( 'z.name ASC', 'Autore asc' );
	$ordering[] = mosHTML::makeOption( 'z.name DESC', 'Autore disc' );
	$ordering[] = mosHTML::makeOption( 'c.state ASC', 'Pubblicazione asc' );
	$ordering[] = mosHTML::makeOption( 'c.state DESC', 'Pubblicazione disc' );
	$ordering[] = mosHTML::makeOption( 'c.access ASC', 'Accesso asc' );
	$ordering[] = mosHTML::makeOption( 'c.access DESC', 'Accesso disc' );
...
function edit( $uid, $option ) {
	...
	if ($uid) {
		...
		if ($row->isCheckedOut( $my->id )) {
			mosErrorAlert( "Il modulo ".$row->title." al momento in fase di modifica da un altro amministratore" );
		}
...
function save( $option, $task ) {
	...
	switch ( $task ) {
		...
		case 'save':
			$msg = 'Contenuto scritto salvato correttamente';
			mosRedirect( 'index2.php?option='. $option, $msg );
			break;

		case 'apply':
		default:
			$msg = 'Modifiche a contenuto scritto salvate correttamente';
...
function changeState( $cid=null, $state=0, $option ) {
	...
	if ( $state == "1" ) {
		$msg = $total ." Articolo(i) pubblicati correttamente";
	} else if ( $state == "0" ) {
		$msg = $total ." Articolo(i) sospesi correttamente";
	}
...
function resethits( $option, $id ) {
	...
	$msg = 'Reset visite eseguito correttamente';
	mosRedirect( 'index2.php?option='. $option .'&task=edit&hidemainmenu=1&id='. $row->id, $msg );
}
...
function menuLink( $option, $id ) {
	...
	$msg = $link .' (Collegamento - Contenuto Statico) nel menu: '. $menu .' creato correttamente';
