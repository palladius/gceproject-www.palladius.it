...
switch ($task) {
	...
	case 'clean_cache':
		...
		mosRedirect( 'index2.php', 'Pulizia cache contenuti completata' );
		break;

	case 'clean_all_cache':
		mosCache::cleanCache();
		mosRedirect( 'index2.php', 'Pulizia cache completata' );
		break;

	case 'redirect':
		$goto = strval( strtolower( mosGetParam( $_REQUEST, 'link' ) ) );
		if ($goto == 'null') {
			$msg = 'Non esistono collegamenti associati a questo oggetto';
