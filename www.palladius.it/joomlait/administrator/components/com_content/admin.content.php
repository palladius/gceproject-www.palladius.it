...
function editContent( $uid=0, $sectionid=0, $option ) {
	...
	if ($uid) {
		...
		if ($row->state < 0) {
			mosRedirect( 'index2.php?option=com_content&sectionid='. $row->sectionid, 'Impossibile modificare un articolo archiviato' );
		}
	}

	// fail if checked out not by 'me'
	if ($row->checked_out && $row->checked_out != $my->id) {
		mosRedirect( 'index2.php?option=com_content', 'Il modulo '. $row->title .' al momento in fase di modifica da parte di un amministratore' );
	}
	...
	$database->setQuery( $query );
	if ( $sectionid == 0 ) {
		$sections[] = mosHTML::makeOption( '-1', 'Seleziona sezione', 'id', 'title' );
	...
	foreach($sections as $section) {
 	...
 	// get list of categories
  	if ( !$row->catid && !$row->sectionid ) {
 		$categories[] 		= mosHTML::makeOption( '-1', 'Seleziona categoria', 'id', 'name' );
...
function saveContent( $sectionid, $task ) {
	...
	if (trim( $row->publish_down ) == 'Mai' || trim( $row->publish_down ) == '') {
...
function changeContent( $cid=null, $state=0, $option ) {
	...
	if (count( $cid ) < 1) {
		$action = $state == 1 ? 'pubblicare' : ($state == -1 ? 'archiviare' : 'sospendere');
	...
	switch ( $state ) {
		case -1:
			$msg = $total .' Oggetto(i) correttamente archiviato';
			break;

		case 1:
			$msg = $total .' Oggetto(i) correttamente pubblicato';
			break;

		case 0:
		default:
			if ( $task == 'unarchive' ) {
				$msg = $total .' Oggetto(i) correttamente ripristinato';
			} else {
				$msg = $total .' Oggetto(i) correttamente sospeso';
			}
			break;
	}
...
function toggleFrontPage( $cid, $section, $option ) {
	...
	if (count( $cid ) < 1) {
		echo "<script> alert('Seleziona un oggetto da modificare'); window.history.go(-1);</script>\n";
		exit;
	}
...
function removeContent( &$cid, $sectionid, $option ) {
	...
	$msg 	= $total ." Oggetto(i) inseriti nel cestino";
...
function moveSectionSave( &$cid, $sectionid, $option ) {
	...
	if (!$newsect && !$newcat ) {
		mosRedirect( "index.php?option=com_content&sectionid=$sectionid&mosmsg=Avvenuto un errore" );
	}
	...
	$msg = $total. ' Oggetto(i) spostati con successo nella Sezione: '. $section .', Categoria: '. $category;
...
function copyItemSave( $cid, $sectionid, $option ) {
	...
	$msg = $total. ' Oggetto(i) copiati con successo nella Sezione: '. $section .', Categoria: '. $category;
	mosRedirect( 'index2.php?option='. $option .'&sectionid='. $sectionid .'&mosmsg='. $msg );
}
...
function menuLink( $redirect, $id ) {
	...
	$msg = $link .' (Collegamento - Contenuto Statico) in menu: '. $menu .' creato correttamente';
	mosRedirect( 'index2.php?option=com_content&sectionid='. $redirect .'&task=edit&hidemainmenu=1&id='. $id, $msg );
}
