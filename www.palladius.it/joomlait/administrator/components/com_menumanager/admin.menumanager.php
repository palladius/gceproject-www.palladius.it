...
function saveMenu() {
	...
	if ( $old_menutype == 'mainmenu' ) {
		if ( $menutype != 'mainmenu' ) {
			echo "<script> alert('Impossibile rinominare il menu \'mainmenu\' in quanto parte integrante per le operazioni di  Joomla'); window.history.go(-1); </script>\n";
			exit;
		}
	}

	// controlla gli ' nel nome menu
	if (strstr($menutype, '\'')) {
		echo "<script> alert('Il nome menu non deve contenere un \''); window.history.go(-1); </script>\n";
		exit;
	}
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menutype ) {
			echo "<script> alert('Un menu con questo nome esiste nel database - dobbiamo inserire un unico nome per il menu'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	switch ( $new ) {
		...
		// create a new module for the new menu
			...
			$msg = 'Nuovo menu creato [ '. $menutype .' ]';
			break;
		...
		// change menutype of all menuitems using old menutype
			...
			$msg = 'Voci di menu & Moduli aggiornati';
			break;
	}
...
function deleteConfirm( $option, $type ) {
	...
	if ( $type == 'mainmenu' ) {
		echo "<script> alert('Impossibile cancellare il menu \'mainmenu\' in quanto menu appartenete al core'); window.history.go(-1); </script>\n";
		exit();
	}
...
function deleteMenu( $option, $cid, $type ) {
	...
	if ( $type == 'mainmenu' ) {
		echo "<script> alert('Impossibile cancellare il menu \'mainmenu\' in quanto menu appartenente al core'); window.history.go(-1); </script>\n";
		exit();
	}
...
function copyMenu( $option, $cid, $type ) {
	...
	$menu_name 		= stripslashes( strval( mosGetParam( $_POST, 'menu_name', 'Nuovo Menu' ) ) );
	$module_name 	= stripslashes( strval( mosGetParam( $_POST, 'module_name', 'Nuovo Modulo' ) ) );
	...
	foreach ( $menus as $menu ) {
		...
		if ( $params->menutype == $menu_name ) {
			echo "<script> alert('Un menu con questo nome esiste nel database - dobbiamo inserire un nome unico per il menu'); window.history.go(-1); </script>\n";
			exit;
		}
	}
	...
	$msg = 'Copia del Menu `'. $type .'` creata, consiste di '. $total .' oggetti';
	mosRedirect( 'index2.php?option=' . $option, $msg );
}
