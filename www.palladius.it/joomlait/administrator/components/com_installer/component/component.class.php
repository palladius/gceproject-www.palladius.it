	...
	*/
	function install($p_fromdir = null) {
		...
		if (file_exists($this->elementDir())) {
			$this->setError( 1, 'Un altro componente sta utilizzando la cartella: "' . $this->elementDir() . '"' );
			return false;
		}

		if(!file_exists($this->elementDir()) && !mosMakePath($this->elementDir())) {
			$this->setError( 1, 'Creazione cartella fallita "' . $this->elementDir() . '"' );
			return false;
		}

		if(!file_exists($this->componentAdminDir()) && !mosMakePath($this->componentAdminDir())) {
			$this->setError( 1, 'Creazione cartella fallita "' . $this->componentAdminDir() . '"' );
			return false;
		}
		...
		if (!is_null($query_element)) {
			...
			foreach($queries as $query)
			{
				...
				if (!$database->query())
				{
					$this->setError( 1, "Errore SQL " . $database->stderr( true ) );
					return false;
				}
			}
		}
		...
		if (!is_null($installfile_elemet)) {
			...
			if (!file_exists($this->componentAdminDir().$installfile_elemet->getText())) {
				if(!$this->copyFiles($this->installDir(), $this->componentAdminDir(), array($installfile_elemet->getText())))  			{
					$this->setError( 1, 'Impossibile copiare il file installazione PHP.' );
					return false;
				}
			}
		...
		if(!is_null($uninstallfile_elemet)) {
			if (!file_exists($this->componentAdminDir().$uninstallfile_elemet->getText())) {
				if(!$this->copyFiles($this->installDir(), $this->componentAdminDir(), array($uninstallfile_elemet->getText()))) {
					$this->setError( 1, 'Impossibile copiare il file disinstallazione PHP' );
					return false;
				}
			}
		}
	...
	*/
	function uninstall( $cid, $option, $client=0 ) {
		...
		if (!$database->loadObject( $row )) {
			HTML_installer::showInstallMessage($database->stderr(true),'Errore - Disinstallazione',
				$this->returnTo( $option, 'component', $client ) );
			exit();
		}

		if ($row->iscore) {
			HTML_installer::showInstallMessage("Il Componente $row->name &egrave; un componente del core, impossibile cancellarlo.<br />Puoi solo renderlo non pubblicato se non desideri utilizzarlo", 'Errore - Disinstallazione',
				$this->returnTo( $option, 'component', $client ) );
			exit();
		}
		...
		if (count($filesindir) > 0) {
			...
			/*
			HTML_installer::showInstallMessage( 'Impossibile trovare file setup XML  in '.$mosConfig_absolute_path.'/administrator/components/'.$row->option,
		...
		if (trim( $row->option )) {
			...
			HTML_installer::showInstallMessage( 'Campo opzione vuoto, non posso rimuovere i file', 'Errore - Disinstallazione', $option,'component');
			exit();
		}
