	...
	function uninstall( $id, $option, $client=0 ) {
		...
		if (trim( $id )) {
			if (is_dir( $path )) {
				...
				HTML_installer::showInstallMessage( 'La cartella non esiste, impossibile rimuovere i file', 'Errore - Disinstallazione',
					$this->returnTo( $option, 'template', $client ) );
			}
		} else {
			HTML_installer::showInstallMessage( 'Template id vuoto, impossibile rimuovere i file', 'Errore - Disinstallazione',
				$this->returnTo( $option, 'template', $client ) );
			exit();
		}
	}
