	...
	*/
	function install( $p_fromdir = null ) {
		...
		if(!file_exists($this->elementDir()) && !mosMakePath($this->elementDir())) {
			$this->setError( 1, 'Fallita creazione directory "' . $this->elementDir() . '"' );
			return false;
		}

		if ($this->parseFiles( 'files', 'mambot', 'Nessun file indicato come file mambot' ) === false) {
			return false;
		}
		...
		if (!$id) {
			...
			$this->setError( 1, 'Il mambot "' . $this->elementName() . '" esiste in archivio!' );
			return false;
		}
	...
	*/
	function uninstall( $id, $option, $client=0 ) {
		...
		if ($database->getErrorNum()) {
			HTML_installer::showInstallMessage( $database->stderr(), 'Errore - Disinstallazione',
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}
		if ($row == null) {
			HTML_installer::showInstallMessage( 'Oggetto id non valido', 'Errore - Disinstallazione',
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}

		if (trim( $row->folder ) == '') {
			HTML_installer::showInstallMessage( 'Campo cartella vuoto, impossibile rimuovere i file', 'Errore - Disinstallazione',
			$this->returnTo( $option, 'mambot', $client ) );
			exit();
		}
		...
		if (file_exists( $xmlfile )) {
			...
			if ($this->i_xmldoc->loadXML( $xmlfile, false, true )) {
				...
				if (!is_null( $files_element )) {
					...
					foreach ($files as $file) {
						...
						if (file_exists( $basepath . $filename )) {
							...
							if ($subpath != '' && $subpath != '.' && $subpath != '..') {
								echo '<br />cANCELLAZIONE: '. $basepath . $subpath;
								$result = deldir(mosPathName( $basepath . $subpath . '/' ));
							} else {
								echo '<br />Cancellazione: '. $basepath . $filename;
					...
					echo "Cancellazione XML File: $xmlfile";
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage( $row->name .' &egrave; un elemento del core, e non pu&ograve; essere rimosso.<br />Se non volete utilizzarlo potete solo non pubblicarlo',
			'Errore -  disinstallazione', $this->returnTo( $option, 'mambot', $client ) );
			exit();
		}
