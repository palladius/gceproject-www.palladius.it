	...
	*/
	function extractArchive() {
		...
		if (eregi( '.zip$', $archivename )) {
			...
			if($ret == 0) {
				$this->setError( 1, 'Errore non risolvibile "'.$zipfile->errorName(true).'"' );
				return false;
			}
		} else {
	...
	*/
	function findInstallFile() {
		...
		if (count( $files ) > 0) {
			...
			$this->setError( 1, 'ERRORE: Impossibile trovare un file setup Joomla! XML nel pacchettto.' );
			return false;
		} else {
			$this->setError( 1, 'ERRORE: Impossibile trovare un file setup XML nel pacchetto.' );
			return false;
		}
	}
	/**
	...
	*/
	function readInstallFile() {
		...
		if ($this->installFilename() == "") {
			$this->setError( 1, 'Nessun nome file specificato' );
			return false;
		}
		...
		if ($root->getTagName() != 'mosinstall') {
			$this->setError( 1, 'Il file :"' . $this->installFilename() . '" non &egrave; un valido file installazione Joomla!' );
			return false;
		}
	...
	*/
	function preInstallCheck( $p_fromdir, $type ) {
		...
		if (!$this->readInstallFile()) {
			$this->setError( 1, 'File installazione non trovato:<br />' . $this->installDir() );
			return false;
		}

		if ($this->installType() != $type) {
			$this->setError( 1, 'XML setup file non  per "'.$type.'".' );
			return false;
		}
			...
			if (basename( $file->getText() ) != $file->getText()) {
				...
				if ($adminFiles){
					if (!mosMakePath( $this->componentAdminDir(), $newdir )) {
						$this->setError( 1, 'Creazione cartella fallita "' . ($this->componentAdminDir()) . $newdir . '"' );
						return false;
					}
				} else {
					if (!mosMakePath( $this->elementDir(), $newdir )) {
						$this->setError( 1, 'Creazione cartella fallita "' . ($this->elementDir()) . $newdir . '"' );
						return false;
					}
				}
			}
		...
		if (is_array( $p_files ) && count( $p_files ) > 0) {
			foreach($p_files as $_file) {
				...
				if (!file_exists( $filesource )) {
					$this->setError( 1, "File $filesource non esiste!" );
					return false;
				} else if (file_exists( $filedest ) && !$overwrite) {
					$this->setError( 1, "Esiste un file chiamato $filedest - Stai tentando di installare lo stesso file due volte?" );
					return false;
				} else {
					...
					if( !( copy($filesource,$filedest) && mosChmod($filedest) ) ) {
						$this->setError( 1, "Copia file fallita: $filesource to $filedest" );
						return false;
					}
				}
			}
