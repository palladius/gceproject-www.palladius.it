...
class HTML_module {
	...
	function showInstalledModules( &$rows, $option, &$xmlfile, &$lists ) {
		if (count($rows)) {
			...
			<tr>
				<td colspan="3">
				Solo i moduli mostrati possono essere rimossi - alcuni moduli del Core non possono essere rimossi.
				<br /><br />
				</td>
			</tr>
			</table>
			...
			for ($i = 0, $n = count( $rows ); $i < $n; $i++) {
				...
				<tr class="<?php echo "row$rc"; ?>">
					...
					<td>
					<?php echo $row->client_id == "0" ? 'Sito' : 'Amministratore'; ?>
					</td>
					<td>
			...
			?>
			<td class="small">
			Nessun modulo custom installato
			</td>
			<?php
		}
		?>
