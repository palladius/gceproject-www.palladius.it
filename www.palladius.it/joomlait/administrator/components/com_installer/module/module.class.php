...
class mosInstallerModule extends mosInstaller {
	...
	function install( $p_fromdir = null ) {
		...
		if ($mosinstall->getAttribute( 'client' )) {
			...
			if (!in_array( $mosinstall->getAttribute( 'client' ), $validClients )) {
				$this->setError( 1, 'Tipo client sconosciuto ['.$mosinstall->getAttribute( 'client' ).']' );
				return false;
			}
			$client = 'admin';
		}
		...
		if ($this->parseFiles( 'files', 'module', 'Nessun file è marcato come file modulo' ) === false) {
			return false;
		}
		...
		if (!$id) {
			...
			$this->setError( 1, 'Il Modulo "' . $this->elementName() . '" esiste in archivio!' );
			return false;
		}
	...
	*/
	function uninstall( $id, $option, $client=0 ) {
		...
		if ($row->iscore) {
			HTML_installer::showInstallMessage( $row->title .'&egrave; un modulo del core, e non pu&ograve; essere rimosso.<br />Potete solo non pubblicarlo sen desiderate utilizzarlo', 'Errore - Disinstallazione', $this->returnTo( $option, 'module', $row->client_id ? '' : 'admin' ) );
			exit();
		}
    		...
    		if (file_exists( $xmlfile )) {
    			...
    			if ($this->i_xmldoc->loadXML( $xmlfile, false, true )) {
    				...
    				if (!is_null( $files_element )) {
    					...
    					echo "Cancellazione file XML: $xmlfile";
