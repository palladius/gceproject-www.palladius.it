...
class HTML_downloads {
	...
	function frontHTML( &$categories, $submitok, $submit_text, &$repository, &$remUser )
	{
		...
		?>
		<table width='100%' border='0' cellspacing='0' cellpadding='0' ><tr><td width='100%' align='center'><span class="small"><a href="http://www.remository.com" target="_blank">Remository 3.21.</a> is technology by <a href="http://www.black-sheep-research.com" target="_blank">Black Sheep Research</a></span></td></tr></table>
		<?php
	}
...
function downloadHTML( &$file, &$repository, $userid ) {
	...
	function fileinfoHTML( &$file, &$remUser ) {
		...
		<table border="0" cellpadding="0" cellspacing="0" width="100%">
					...
					if ($file->filedate<>'') infoDisplay (_DOWN_FILE_DATE, date($repository->Date_Format,revertDate($file->filedate)));
	...
	function addfileHTML( &$remUser, &$repository, &$file, $filetemp=null, $filetemphash=null, $oldid=0 )
	{
		...
		if ( $remUser->isAdmin() OR ($repository->User_Remote_Files AND $remUser->isLogged())) {
			$remoteok = true;
