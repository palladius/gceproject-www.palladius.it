...
if ($action == 'wipe'){
    ...
    @mkdir ($mosConfig_absolute_path.'/'.$zoom->_CONFIG['imagepath'], 0755);
    @mkdir ($mosConfig_absolute_path.'/'.$zoom->_CONFIG['imagepath']."watermarks", 0755);
	@$zoom->platform->chmod($mosConfig_absolute_path.'/'.$zoom->_CONFIG['imagepath'], '0755');
    @$zoom->platform->chmod($mosConfig_absolute_path.'/'.$zoom->_CONFIG['imagepath']."watermarks", '0755');
...
if ($theButton == 'save') {
	...
	if ($zoom->saveConfig()) {
		...
		@$zoom->platform->chmod($css_popup_file, '0755');
		@$zoom->platform->chmod($css_zoom_file, '0755');
	    ...
	    if(strlen($zoom->_CONFIG['safemodeversion']) > 0){
			...
			@$zoom->platform->chmod($ftp_file, '0755');
	...
	if (!empty($_FILES['new_wm_file']['name'])) {
		...
		if ($zoom->isImage($tag)) {
			if (move_uploaded_file($_FILES['new_wm_file']['tmp_name'], $mosConfig_absolute_path."/".$zoom->_CONFIG['imagepath']."/watermarks/".$_FILES['new_wm_file']['name'])) {
				@$zoom->platform->chmod($mosConfig_absolute_path."/".$zoom->_CONFIG['imagepath']."/watermarks/".$_FILES['new_wm_file']['name'], '0755');
