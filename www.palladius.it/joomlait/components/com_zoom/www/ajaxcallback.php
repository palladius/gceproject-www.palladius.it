	...
	switch ($task) {
	   ...
	   case 'catsmgr_new' :
           ...
           if (!empty($name)){
            	...
            	if ($zoom->createdir($zoom->_CONFIG['imagepath'].$mkdir, '755')){
            		$zoom->createdir($zoom->_CONFIG['imagepath'].$mkdir."/thumbs", '755');
            		$zoom->createdir($zoom->_CONFIG['imagepath'].$mkdir."/viewsize", '755');
