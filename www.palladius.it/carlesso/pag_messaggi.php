...
if ((Form("OPERAZIONE")) == "CANCELLA")
	...
	$res=getRecordSetByQuery("select count(*) from messaggi where id_figliodi_msg=".Form("ID_MSG"));
...
if ((Form("OPERAZIONE")) == "RENDIUSER") // ereditata dall'index, sono le X sugli ultimi 10 utenti...
	...
	getRecordSetByQuery("uPdAtE loginz SET [m_bguest]=false WHERE id_login=".Form("id_login"))
	or die("errore a userizzare l'utente ".Form("id_login"));

	
	scrivi(rosso("userizzo l'utente ".Form("id_login").", mio caro amministratore..."));
...
if ($trigger)
	...
	if ((Form("messaggio"))=="")
		errore2002(("Messaggio mancante, non inserisco niente (form  vale ".Form("messaggio")."!"));
	else
	{	  // l'utente POSTANTE è spoofabile
...
$nuovoMsg=($str==""); // in tal caso, non esiste un msg letto, solo il nuovo da scrivere...
...
$sqlRES=getRecordSetByQuery("select * from messaggi  m,loginz l WHERE m.id_login = l.id_login AND id_figliodi_msg=".$str);
